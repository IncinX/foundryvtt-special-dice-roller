...
test('it should fail to parse a roll formula', () => {
    const msg = 'Incorrect roll formula 1dx + 4ds! Usage: Any combination of the following letters: h, s, e, n, b, t, +, d, w, g (h = hero, s = superior, e = enhanced, n = normal, b = bad, t = terrible, + = superior defense, d = defense, w = wounds, g = guaranteed wound). To roll multiple dice simply add multiple letters or prepend a number, e.g.: c3ba';
    expect(() => parseFormula('1dx + 4ds', parsers)).toThrow(msg);
});

test('it should parse a simple farhome roll formula with each die', () => {
    const result = parseFormula('hsenbt+dwg', parsers);
